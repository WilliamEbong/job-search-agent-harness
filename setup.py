...
"""One-command setup and doctor for the Job Search Agent Harness.
    ...
    python setup.py            # check, offer installs, print the doctor table
    python setup.py --doctor   # check and report only; change nothing
    python setup.py --yes      # take the recommended answer to every prompt
    python setup.py --quick    # skip the stock-template test compiles

Design rules this file follows, and why:

* **Stdlib only.** setup.py runs *before* dependencies exist, so it cannot
