# discover (Codex stub)

Execute the workflow `discover` exactly as specified in
`.claude/commands/discover.md`, applying `RUNTIME-MAP.md` wherever that file
records a difference between runtimes.

The command file is the single source of truth for the procedure. This stub
exists only because Codex has no slash-command registry — it adds no behaviour
and overrides nothing.

Before starting, read:
1. `AGENTS.md` — orientation, including the plain-language routing table and
   the canonical application-status vocabulary
2. `.claude/commands/discover.md` — the workflow itself
3. `RUNTIME-MAP.md` — §3 (tool-name mapping, including web search), §4 (the
   `.claude/skills/` files Codex must read by path rather than auto-discover)
