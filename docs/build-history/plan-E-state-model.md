...
| Companies of interest | `companies.yaml` — a LIVING list, written by the companies command at any time (onboarding is just the first run): user-named entries + system-researched proposals (from CV + location: big companies + local companies hiring the user's skill set), each proposal approved by the user before it lands; entries can be added, updated, or removed later by the same command; fields per entry: `name, careers_url, source (user\|researched <date>), location_scope, notes` | YAML | company-of-interest search reads it; workbook Shortlist rows carry source |
...
| Board roster | `.agents/skills/*-search/` (one dir per board, `enabled:` flag honored by scrape) — a LIVING roster: new boards added at any time via upstream `/add-portal` (user-supplied URL or system-researched proposal), removed by disabling the flag | skill dirs | scrape scope `boards` reads it |
