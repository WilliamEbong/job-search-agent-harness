...
| Companies of interest | `companies.yaml` — written by the companies command: user-named entries + system-researched proposals (from CV + location: big companies + local companies hiring the user's skill set), each proposal approved by the user before it lands; fields per entry: `name, careers_url, source (user\|researched <date>), location_scope, notes` | YAML | company-of-interest search reads it; workbook Shortlist rows carry source |
