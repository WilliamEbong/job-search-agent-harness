...
| Caveman | **not installed** (exists only as a ponytail benchmark arm). Verified install commands: Claude `claude plugin marketplace add JuliusBrussee/caveman` + `claude plugin install caveman@caveman`; Codex `npx skills add JuliusBrussee/caveman -a codex`; SHA-256-manifest installer | **Owner directive: install during P0 on both runtimes**, verify post-install; setup.py installs for end users; default-ON internal channels only, hard-fenced OFF deliverables |
