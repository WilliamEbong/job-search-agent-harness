# add-template (Codex stub)

Execute the workflow `add-template` exactly as specified in
`.claude/commands/add-template.md`, applying `RUNTIME-MAP.md` wherever that file
...
2. `.claude/commands/add-template.md` — the workflow itself
3. `RUNTIME-MAP.md` — §3 (tool-name mapping), §5 (never print a usage percentage on Codex)
